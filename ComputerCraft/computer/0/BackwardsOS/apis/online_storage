...
function get_metadata()
	local handle = http.get("http://h2896147.stratoserver.net:3000/metadata")
...
function get_ascii_subfile(asciiID, subfileID)
	local handle = http.post("http://h2896147.stratoserver.net:3000/get-ascii-subfile", "foo=" .. json.encode({asciiID = asciiID, subfileID = subfileID}))
